...
type TProps = {
    ...
    className?: string;
    size?: "md" | "lg";
    // "C" = on navy, mirrors SectionTitle's color prop
    color?: "A" | "C";
};

const MottoText = forwardRef<HTMLParagraphElement, TProps>(
    ({ text, pathText, path, className, size, color }, ref) => (
        <p
            className={clsx(
                "tw-font-medium tw-leading-relaxed",
                color === "C" ? "tw-text-cream" : "tw-text-secondary-light",
        ...
        >
            ...
            <Anchor
                path={path}
                className={clsx(
                    "tw-relative tw-py-[3px] tw-font-bold tw-leading-none",
                    color === "C" ? "tw-text-gold after:tw-bg-gold" : "tw-text-primary after:tw-bg-primary",
                    ...
                    "after:tw-absolute after:tw-bottom-0 after:tw-left-0 after:tw-h-px after:tw-w-full after:tw-origin-left after:tw-scale-x-0 after:tw-transition-transform after:tw-delay-75 after:tw-duration-600 after:tw-ease-in-expo after:tw-content-['']",
