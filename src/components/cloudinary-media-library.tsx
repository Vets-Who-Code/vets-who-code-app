...
import { CloudinaryImage, useCloudinaryImages } from "@/hooks/use-cloudinary-images";
...
const CloudinaryMediaLibrary: React.FC<CloudinaryMediaLibraryProps> = ({
    ...
    const { images, loading, error, hasMore, searchImages, loadMore, refresh } =
        useCloudinaryImages({
            folder,
    ...
    const handleSearch = useCallback(
        async (e: React.FormEvent) => {
            e.preventDefault();
            if (searchQuery.trim()) {
                const expression = folder ? `folder:${folder} AND ${searchQuery}` : searchQuery;
