            ...
            {phase === "active" && (
                <div className="tw-space-y-6">
                    ...
                    {history.map((item, i) => (
                        <div
                            key={i}
                            className="tw-rounded-lg tw-bg-white tw-p-5 tw-shadow-sm tw-border tw-border-navy/5"
                        >
                            <div className="tw-flex tw-items-center tw-justify-between tw-mb-2">
                                ...
                                <span className={`tw-font-bold ${scoreColor(item.score)}`}>
                                    {item.score}/10
                                </span>
                            </div>
                            <p className="tw-text-sm tw-text-navy/60 tw-mb-2 tw-pl-6">
                                {item.answer}
                            </p>
                            <div className="tw-pl-6 tw-space-y-2">
                                <p className="tw-text-sm tw-text-ink/80 tw-italic">
                                    {item.feedback}
                                </p>
                                {item.what_landed && (
                                ...
                                {item.stronger_version && (
                                    <details className="tw-text-sm">
                                        <summary className="tw-cursor-pointer tw-text-primary tw-font-medium">
                                            Stronger answer
                                        </summary>
                ...
                <div className="tw-space-y-6">
                    <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                        <div className="tw-flex tw-items-center tw-gap-4 tw-mb-6">
                            ...
                            <div>
                                <h2 className="tw-text-xl tw-font-bold tw-font-mono tw-text-ink">
                                    Interview Complete
                                </h2>
                                <p className="tw-text-sm tw-text-navy/60">
                                    {jobTitle} — {history.length} questions answered
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="tw-space-y-4">
                            {history.map((item, i) => (
                                <div
                                    key={i}
                                    className="tw-border-b tw-border-navy/5 tw-pb-4 last:tw-border-0"
                                >
