...
export default function ResumeScorer() {
    ...
    const handleFileUpload = useCallback(
        async (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            try {
                ...
                // usePdfUpload surfaces the error via `uploadError`, which the UI
                // renders below. Log so devs see it in the console too.
                handleClientError(err, { context: "ResumeScorer:handleFileUpload" });
            }
        },
        [upload]
    );
    ...
    const handleDrop = useCallback(
        (e: React.DragEvent) => {
            ...
            if (file.type === "text/plain") handleTextFileRead(file);
                ...
                upload(file)
                    .then(setResumeText)
                    .catch((err) => handleClientError(err, { context: "ResumeScorer:handleDrop" }));
        },
    ...
    const sanitizeText = (text: string) =>
        text
            ...
            .replace(
                /Please check formatting\.?/gi,
                "Try pasting your resume text directly for best results."
            )
        ...
        <div className="tw-space-y-6">
            ...
            <div className="tw-grid tw-grid-cols-1 lg:tw-grid-cols-2 tw-gap-6">
                <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                    ...
                    >
                        ...
                        {uploading ? (
                            ...
                            <div>
                                ...
                                <button
                                    ...
                                    className="tw-text-xs tw-text-ink/60 hover:tw-text-red-500 tw-mt-1"
                                >
                                    Clear
                                </button>
                            </div>
                        ...
                        {uploadError && (
                            <p className="tw-text-xs tw-text-red-500 tw-mt-2">{uploadError}</p>
                        )}
                    </div>
                    <textarea
                        value={resumeText}
                    ...
                    {resumeText && (
                        <p className="tw-text-xs tw-text-ink/70 tw-mt-1">
                            {resumeText.length.toLocaleString()} characters loaded. Review the text
                            above to make sure it looks right before scoring.
                        </p>
                    )}
                    <div className="tw-mt-3">
                        <label
                            htmlFor="target-role"
                            className="tw-block tw-text-sm tw-font-medium tw-text-ink/80 tw-mb-1"
                        >
                            Target Role
                        </label>
                        <select
                ...
                <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                    <h2 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-3">
                        ...
                        Job Posting
                        <span className="tw-text-xs tw-font-normal tw-text-ink/70 tw-ml-2">
                            optional — enables JD match analysis and job-specific tailoring
                        </span>
                    </h2>
                    <textarea
            ...
            <div className="tw-flex tw-flex-wrap tw-gap-3">
                ...
                >
                    {isScoring ? (
                        ...
                        <span>
                            <i className="fas fa-chart-bar tw-mr-2" />
                            Score Resume
                        </span>
                    )}
                </button>
                ...
                >
                    {isRewriting ? (
                        ...
                        <span>
                            <i className="fas fa-magic tw-mr-2" />
                ...
                {jobDescription.trim() && (
                    <button
                        ...
                        disabled={isTailoring || !resumeText.trim()}
                    ...
                    >
                        {isTailoring ? (
                            ...
                            <span>
                                <i className="fas fa-crosshairs tw-mr-2" />
                ...
                {r && (
                    <button
                        ...
                        className="tw-rounded-md tw-border tw-border-navy/10 tw-px-4 tw-py-2.5 tw-text-sm tw-font-medium tw-text-ink/80 hover:tw-bg-navy/5"
                    >
                ...
                <div className="tw-space-y-6">
                    ...
                    <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                        <div className="tw-flex tw-items-center tw-gap-6 tw-mb-6">
                            <div className="tw-relative tw-flex-shrink-0">
                                <svg
                                    className="tw-w-28 tw-h-28 tw-transform -tw-rotate-90"
                                    viewBox="0 0 120 120"
                                >
                                    <circle
                                        cx="60"
                                    ...
                                    <circle
                                        ...
                                        strokeLinecap="round"
                                        style={{
                                            strokeDasharray: `${(r.overall_score / 100) * 327} 327`,
                                            stroke:
                                                r.overall_score >= 80
                                                    ? "#22c55e"
                                                    : r.overall_score >= 60
                                ...
                                <div className="tw-absolute tw-inset-0 tw-flex tw-flex-col tw-items-center tw-justify-center">
                                    <span
                                        className={`tw-text-3xl tw-font-bold ${scoreTextColor(r.overall_score)}`}
                                    >
                                        {r.overall_score}
                                    </span>
                                    <span
                                        className={`tw-text-xs tw-font-bold tw-rounded-full tw-px-2 tw-py-0.5 ${passFailColor(r.pass_fail)}`}
                                    >
                                        {r.pass_fail}
                                    </span>
                                </div>
                            ...
                            <div className="tw-flex-1">
                                <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-1">
                                    Resume Analysis
                                </h3>
                                <p className="tw-text-sm tw-text-navy/60 tw-mb-2">
                                    Target: {targetRole}
                                </p>
                                <p className="tw-text-sm tw-text-ink/80">
                                    {sanitizeText(r.coaching_feedback)}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-3">
                            {Object.entries(r.dimensions).map(([key, dim]) => {
                                ...
                                return (
                                    <div
                                        key={key}
                                        className="tw-flex tw-items-center tw-gap-3 tw-p-3 tw-rounded-md tw-border tw-border-navy/5"
                                    >
                                        <i
                                            className={`fas ${meta.icon} tw-text-ink/60 tw-w-5 tw-text-center`}
                                        />
                                        <div className="tw-flex-1">
                                            <div className="tw-flex tw-items-center tw-justify-between tw-mb-1">
                                                <span className="tw-text-xs tw-font-medium tw-text-ink/80">
                                                    {meta.label}
                                                </span>
                                                <span
                                                    className={`tw-text-xs tw-font-bold ${scoreTextColor(dim.score)}`}
                                                >
                                            ...
                                            <div className="tw-h-2 tw-bg-gray-200 tw-rounded-full tw-overflow-hidden">
                                                <div
                                                    className={`tw-h-full tw-rounded-full tw-transition-all ${dimColor(dim.score)}`}
                                                    style={{ width: `${dim.score}%` }}
                                                />
                                            </div>
                                            {dim.details && dim.details !== "Could not analyze" && (
                                                <p className="tw-text-xs tw-text-ink/60 tw-mt-1">
                                                    {sanitizeText(dim.details)}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-6">
                        <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                            <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-3 tw-flex tw-items-center tw-gap-2">
                                <span className="tw-w-6 tw-h-6 tw-rounded-full tw-bg-green-100 tw-flex tw-items-center tw-justify-center">
                                    <i className="fas fa-check tw-text-green-600 tw-text-xs" />
                                </span>
                                Strengths
                            </h3>
                        ...
                        <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                            <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-3 tw-flex tw-items-center tw-gap-2">
                                <span className="tw-w-6 tw-h-6 tw-rounded-full tw-bg-amber-100 tw-flex tw-items-center tw-justify-center">
                                    <i className="fas fa-exclamation tw-text-amber-600 tw-text-xs" />
                                </span>
                                Improvements
                            </h3>
                            {r.improvements.length > 0 ? (
                                <ul className="tw-space-y-2">
                                    {r.improvements.map((s: any, i: number) => (
                                        <li
                                            key={i}
                                            className="tw-flex tw-gap-2 tw-text-sm tw-text-ink/80"
                                        >
                                            <i className="fas fa-arrow-up tw-text-amber-500 tw-mt-0.5 tw-flex-shrink-0" />
                                            {renderText(s)}
                                        </li>
                                    ))}
                                </ul>
                    ...
                    {(r.detected_technologies.length > 0 ||
                        ...
                        <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                            <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-4">
                                <i className="fas fa-microchip tw-mr-2 tw-text-primary" />
                                Skills Analysis
                            </h3>
                            <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-3 tw-gap-4">
                                {r.detected_technologies.length > 0 && (
                                    <div>
                                        <h4 className="tw-font-mono tw-text-[10px] tw-font-bold tw-text-ink/60 tw-uppercase tw-tracking-widest tw-mb-2">
                                            Detected
                                        </h4>
                                        <div className="tw-flex tw-flex-wrap tw-gap-1.5">
                                            {r.detected_technologies.map((t, i) => (
                                                <span
                                                    key={i}
                                                    className="tw-rounded-full tw-bg-green-100 tw-text-green-800 tw-px-2.5 tw-py-0.5 tw-text-xs tw-font-medium"
                                                >
                                                    {renderText(t)}
                                                </span>
                                            ))}
                                ...
                                {r.missing_skills.length > 0 && (
                                    <div>
                                        ...
                                        <div className="tw-flex tw-flex-wrap tw-gap-1.5">
                                            {r.missing_skills.map((t, i) => (
                                                <span
                                                    key={i}
                                                    className="tw-rounded-full tw-bg-red-100 tw-text-red-800 tw-px-2.5 tw-py-0.5 tw-text-xs tw-font-medium"
                                                >
                                                    {renderText(t)}
                                                </span>
                                            ))}
                                ...
                                {r.recommended_skills.length > 0 && (
                                    <div>
                                        ...
                                        <div className="tw-flex tw-flex-wrap tw-gap-1.5">
                                            {r.recommended_skills.map((t, i) => (
                                                <span
                                                    key={i}
                                                    className="tw-rounded-full tw-bg-blue-100 tw-text-blue-800 tw-px-2.5 tw-py-0.5 tw-text-xs tw-font-medium"
                                                >
                                                    {renderText(t)}
                                                </span>
                                            ))}
                    ...
                    {r.learning_recommendations.length > 0 && (
                        <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                            ...
                            <ul className="tw-space-y-2">
                                {r.learning_recommendations.map((l: any, i: number) => (
                                    <li key={i} className="tw-text-sm tw-text-ink/80">
                                        {typeof l === "string" ? (
                                            <span className="tw-flex tw-gap-2">
                                                <i className="fas fa-book tw-text-primary tw-mt-0.5" />
                                                {l}
                                            </span>
                                        ) : (
                                            <div className="tw-flex tw-gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    {l.reason && (
                                                        <span className="tw-text-navy/60">
                                                            {" "}
                                                            — {l.reason}
                                                        </span>
                                                    )}
                                                    {l.resource && (
                                                        <a
                                                            ...
                                                            className="tw-block tw-text-xs tw-text-primary hover:tw-underline tw-mt-0.5"
                                                        >
                                                            {l.resource}
                                                        </a>
                                                    )}
                    ...
                    {r.weak_verbs.length > 0 && (
                        <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                            <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-3">
                                <i className="fas fa-pen tw-mr-2 tw-text-amber-500" />
                                Weak Verb Detection
                            </h3>
                            <div className="tw-space-y-2">
                                {r.weak_verbs.map((wv, i) => (
                                    <div
                                        key={i}
                                        className="tw-flex tw-items-center tw-gap-3 tw-text-sm tw-p-2 tw-rounded tw-bg-amber-50"
                                    >
                                        <span className="tw-font-mono tw-text-red-600 tw-line-through">
                                            {wv.verb}
                                        </span>
                                        {wv.suggestion && (
                                            <>
                                                <i className="fas fa-arrow-right tw-text-ink/60" />
                                                <span className="tw-font-mono tw-text-green-700 tw-font-medium">
                                                    {wv.suggestion}
                                                </span>
                                            </>
                                        )}
                                        {wv.context && (
                                            <span className="tw-text-xs tw-text-ink/60 tw-ml-auto">
                                                {wv.context}
                                            </span>
                                        )}
                                    </div>
                    ...
                    {r.harvard_violations.length > 0 && (
                        <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                            <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-3">
                                <i className="fas fa-align-left tw-mr-2 tw-text-red-500" />
                                Formatting Violations
                            </h3>
                            <ul className="tw-space-y-1.5">
                                {r.harvard_violations.map((v: any, i: number) => (
                                    <li
                                        key={i}
                                        className="tw-flex tw-gap-2 tw-text-sm tw-text-ink/80"
                                    >
                                        <i className="fas fa-times-circle tw-text-red-400 tw-mt-0.5" />
                                        {sanitizeText(renderText(v))}
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    {r.fraud_detection.overall_flag && (
                        <div className="tw-rounded-lg tw-bg-red-50 tw-border tw-border-red-200 tw-p-6">
                            <h3 className="tw-font-bold tw-text-red-800 tw-mb-3">
                                <i className="fas fa-shield-alt tw-mr-2" />
                                ...
                                {[
                                    {
                                        ...
                                        val: r.fraud_detection.keyword_stuffing,
                                    },
                                    {
                                        key: "hidden_text_suspected",
                                        label: "Hidden Text",
                                        val: r.fraud_detection.hidden_text_suspected,
                                    },
                                    {
                                        key: "copy_paste_from_jd",
                                        label: "Copy/Paste from JD",
                                        val: r.fraud_detection.copy_paste_from_jd,
                                    },
                                    {
                                        key: "acronym_stacking",
                                        label: "Acronym Stacking",
                                        val: r.fraud_detection.acronym_stacking,
                                    },
                                ].map((f) => (
                                    <div
                                        key={f.key}
                                        className={`tw-text-center tw-p-2 tw-rounded ${f.val ? "tw-bg-red-100" : "tw-bg-green-50"}`}
                                    >
                                        <i
                                            className={`fas ${f.val ? "fa-exclamation-triangle tw-text-red-500" : "fa-check tw-text-green-500"} tw-mb-1`}
                                        />
                                        <p className="tw-text-xs tw-font-medium">{f.label}</p>
                                    </div>
                                ))}
                            </div>
                            {r.fraud_detection.details && (
                    ...
                    {(r.employment_red_flags.gaps_over_6_months.length > 0 ||
                        ...
                        <div className="tw-rounded-lg tw-bg-amber-50 tw-border tw-border-amber-200 tw-p-6">
                            <h3 className="tw-font-bold tw-text-amber-800 tw-mb-3">
                                <i className="fas fa-flag tw-mr-2" />
                                Employment Red Flags
                            </h3>
                            <div className="tw-space-y-2 tw-text-sm">
                                {r.employment_red_flags.gaps_over_6_months.length > 0 && (
                                    <p className="tw-text-amber-700">
                                ...
                                {r.employment_red_flags.details && (
                                    <p className="tw-text-ink/60 tw-mt-1">
                                        {r.employment_red_flags.details}
                                    </p>
                                )}
                            </div>
                    ...
                    {r.jd_match && (
                        <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                            <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-4 tw-flex tw-items-center tw-gap-2">
                                ...
                                JD Match Report
                                <span
                                    className={`tw-ml-auto tw-text-lg tw-font-bold ${scoreTextColor(r.jd_match.match_score)}`}
                                >
                                    {r.jd_match.match_score}%
                                </span>
                            </h3>
                            <div className="tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-4 tw-mb-4">
                                {r.jd_match.matched_skills.length > 0 && (
                                    <div>
                                        <h4 className="tw-text-xs tw-font-bold tw-text-green-700 tw-uppercase tw-mb-2">
                                            Matched Skills
                                        </h4>
                                        <div className="tw-flex tw-flex-wrap tw-gap-1.5">
                                ...
                                {r.jd_match.missing_skills.length > 0 && (
                                    <div>
                                        <h4 className="tw-text-xs tw-font-bold tw-text-red-700 tw-uppercase tw-mb-2">
                                            Missing Skills
                                        </h4>
                                        <div className="tw-flex tw-flex-wrap tw-gap-1.5">
                                            {r.jd_match.missing_skills.map((s: any, i: number) => (
                                                <span
                                                    key={i}
                                                    className="tw-rounded-full tw-bg-red-100 tw-text-red-800 tw-px-2.5 tw-py-0.5 tw-text-xs tw-font-medium"
                                                >
                            ...
                            {r.jd_match.recommendations.length > 0 && (
                                <div>
                                    ...
                                    <ul className="tw-space-y-1">
                                        {r.jd_match.recommendations.map((rec: any, i: number) => (
                                            <li
                                                key={i}
                                                className="tw-flex tw-gap-2 tw-text-sm tw-text-ink/80"
                                            >
                                                <i className="fas fa-lightbulb tw-text-yellow-500 tw-mt-0.5" />
                                                {renderText(rec)}
                                            </li>
                                        ))}
                                    </ul>
                    ...
                    {r.rewrite_suggestions.length > 0 && (
                        <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                            <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-4 tw-flex tw-items-center tw-gap-2">
                                <span className="tw-w-6 tw-h-6 tw-rounded-full tw-bg-blue-100 tw-flex tw-items-center tw-justify-center">
                                    <i className="fas fa-pen tw-text-blue-600 tw-text-xs" />
                                </span>
                                Bullet Rewrites
                            </h3>
                            <div className="tw-space-y-4">
                                {r.rewrite_suggestions.map((rw, i) => (
                                    <div
                                        key={i}
                                        className="tw-border tw-border-navy/10 tw-rounded-lg tw-overflow-hidden"
                                    >
                                        <div className="tw-bg-red-50 tw-px-4 tw-py-3 tw-border-b tw-border-navy/10">
                                            <span className="tw-text-xs tw-font-bold tw-text-red-600 tw-uppercase">
                                                Before
                                            </span>
                                            <p className="tw-text-sm tw-text-red-800 tw-line-through tw-mt-1">
                                                {rw.original_bullet}
                                            </p>
                                        </div>
                                        <div className="tw-bg-green-50 tw-px-4 tw-py-3 tw-border-b tw-border-navy/10">
                                            <span className="tw-text-xs tw-font-bold tw-text-green-600 tw-uppercase">
                                                After
                                            </span>
                                            <p className="tw-text-sm tw-text-green-800 tw-font-medium tw-mt-1">
                                                {rw.suggested_bullet}
                                            </p>
                                        </div>
                                        <div className="tw-px-4 tw-py-3 tw-bg-navy/5">
                                            <p className="tw-text-xs tw-text-ink/60 tw-italic">
                                                <i className="fas fa-lightbulb tw-text-yellow-500 tw-mr-1" />
                                                {rw.reason}
                                            </p>
                                        </div>
                                    </div>
                ...
                <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                    <div className="tw-flex tw-items-center tw-justify-between tw-mb-4">
                        <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-flex tw-items-center tw-gap-2">
                            <i className="fas fa-magic tw-text-primary" />
                            Fixed Resume
                        </h3>
                        <div className="tw-flex tw-items-center tw-gap-3">
                            <div className="tw-text-center">
                                ...
                                <span
                                    className={`tw-text-lg tw-font-bold ${scoreTextColor(rewriteResult.original_score)}`}
                                >
                                    {rewriteResult.original_score}
                                </span>
                            </div>
                            ...
                            <div className="tw-text-center">
                                ...
                                <span
                                    className={`tw-text-lg tw-font-bold ${scoreTextColor(rewriteResult.rewrite_score)}`}
                                >
                                    {rewriteResult.rewrite_score}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="tw-flex tw-gap-3 tw-mt-4">
                        <button
                            onClick={() => handleDownloadRewrittenResume("txt")}
                            className="tw-rounded-md tw-bg-primary tw-px-5 tw-py-2 tw-text-sm tw-font-medium tw-text-white hover:tw-bg-primary-dark tw-transition-colors"
                        >
                            <i className="fas fa-download tw-mr-2" />
                            Download as TXT
                        </button>
                        <button
                            onClick={() => handleDownloadRewrittenResume("md")}
                            className="tw-rounded-md tw-border tw-border-navy/10 tw-px-5 tw-py-2 tw-text-sm tw-font-medium tw-text-ink/80 hover:tw-bg-navy/5 tw-transition-colors"
                        >
                            <i className="fas fa-download tw-mr-2" />
            ...
            {tailorResult && (
                <div className="tw-rounded-lg tw-bg-white tw-p-6 tw-shadow-sm">
                    <h3 className="tw-font-mono tw-text-xs tw-font-bold tw-uppercase tw-tracking-widest tw-text-navy/60 tw-mb-3 tw-flex tw-items-center tw-gap-2">
                        <i className="fas fa-crosshairs tw-text-primary" />
                        Tailored for Job Posting
                    </h3>
                    <pre className="tw-rounded-md tw-bg-navy/5 tw-p-4 tw-text-sm tw-text-ink/80 tw-whitespace-pre-wrap tw-border tw-border-navy/10 tw-max-h-[500px] tw-overflow-y-auto">
                        {tailorResult.response}
                    </pre>
                </div>
            )}
        </div>
