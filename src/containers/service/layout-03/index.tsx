    ...
    return (
        <Section className="service-area" space={space} bg={bg}>
            <div className="tw:container">
                ...
                <div className="tw:relative tw:top-[-90px] tw:z-10 tw:mb-[-90px] tw:grid tw:grid-cols-1 tw:gap-7.5 tw:md:grid-cols-2 tw:lg:grid-cols-3 tw:xl:top-[-125px] tw:xl:mb-[-125px]">
                    {items?.map((item) => (
                        <AnimatedServiceCard
                            key={item.id}
                            title={item.title}
