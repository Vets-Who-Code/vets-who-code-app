...
const TheoryOfChange: PageWithLayout = ({ data }) => {
    ...
    return (
        <>
            ...
            {theoryOfChangeData && (
                <div className="tw:py-15 tw:md:py-20 tw:lg:py-28">
                    <div className="tw:container">
                        {theoryOfChangeData.section_title?.title && (
                            <h2 className="tw:mb-4 tw:text-center tw:text-3xl tw:font-bold tw:md:text-4xl tw:lg:text-5xl">
                                {theoryOfChangeData.section_title.title}
                            </h2>
                        )}
                        {theoryOfChangeData.section_title?.subtitle && (
                            <p className="tw:mb-10 tw:text-center tw:text-lg tw:md:mb-16">
                                {theoryOfChangeData.section_title.subtitle}
                            </p>
                        )}
                        {theoryOfChangeData.items?.map((item, itemIndex) => (
                            <div key={item.id} className="tw:mb-16 tw:last:mb-0">
                                    ...
                                    {item.texts?.map((text, index) => {
                                        ...
                                        if (item.texts && item.texts.length > 1 && index > 0) {
                                            ...
                                            return (
                                                <div
                                                    key={text.id}
                                                    className="tw:mb-3 tw:flex tw:last:mb-0"
                                                >
