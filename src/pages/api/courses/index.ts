...
async function handleGet(req: AuthenticatedRequest, res: NextApiResponse) {
    try {
        const { category, difficulty, isPublished, search, limit = "10", offset = "0" } = req.query;
        ...
        const moduleIdToCourseId = Object.fromEntries(
            moduleCourseMap.map((m) => [m.id, m.courseId])
        );
        ...
        for (const lc of lessonCounts) {
            ...
            if (courseId) {
                courseIdToLessonCount[courseId] =
                    (courseIdToLessonCount[courseId] || 0) + lc._count._all;
            }
        }
