...
export const TechStack = ({ techStack }: TechStackProps) => {
    return (
        <div className="tw:mb-2 tw:flex tw:flex-wrap tw:gap-1">
            {techStack.map((tech) => (
                <div className="tw:m-0 tw:rounded-md tw:bg-secondary/90 tw:p-0 tw:px-2 tw:py-1 tw:text-sm tw:text-white">
                    {tech}
                </div>
            ))}
        </div>
    ...
    return (
        <div className="tw:mb-3 tw:flex tw:items-start tw:gap-1 tw:text-black">
            <Link
                ...
                className="tw:flex tw:w-fit tw:items-center tw:justify-center tw:space-x-1 tw:rounded-md tw:border-2 tw:border-secondary tw:px-2 tw:py-1 tw:text-center tw:text-secondary tw:hover:bg-secondary tw:hover:text-white"
            >
                <GitHubLogoIcon />
...
export const TopContributors = ({ contributors }: TopContributorsProps) => {
    return (
        <>
            ...
            <div className="tw:grid-cols-2 tw:space-y-1 tw:text-secondary tw:md:grid-cols-1">
                {contributors.map((contributor) => {
                    return (
                        <Link
                            href={contributor.html_url}
                            target="_blank"
                            className="tw:flex tw:min-w-56 tw:gap-2 tw:rounded-md tw:border-2 tw:border-secondary/35 tw:bg-secondary/20 tw:px-2 tw:py-1 tw:hover:cursor-pointer tw:hover:bg-secondary/100 tw:hover:text-white"
                        >
...
export const ProjectDetailModal = ({ project, className }: ProjectModalProps) => {
    ...
    return (
        <div className="tw:m-0 tw:max-w-fit tw:py-2 tw:md:flex tw:md:space-x-2 tw:md:px-4">
            {/* Left Column */}
            <motion.div
                className={clsx(
                    "tw:w-full tw:flex-col tw:items-center tw:justify-center tw:space-y-2 tw:py-3 tw:md:w-2/5 tw:md:min-w-80 tw:md:max-w-fit tw:md:items-start tw:md:justify-normal",
                    className
                )}
            ...
            {/* Center divider */}
            <div className="tw:m-0 tw:grid tw:w-full tw:grid-cols-1 tw:grid-rows-1 tw:items-center tw:justify-center tw:p-0 tw:md:w-fit">
                <motion.div
                    ...
                    transition={{ delay, duration }}
                >
                    <div className="tw:h-[1.5px] tw:w-full tw:rounded-md tw:bg-secondary tw:md:h-full tw:md:w-[1.5px]" />
                </motion.div>
                    ...
                    <Dialog.Portal forceMount>
                        ...
                        <Dialog.Content asChild>
                            <motion.div
                                ...
                                className="tw:fixed tw:left-1/2 tw:top-1/2 tw:z-60 tw:grid tw:max-h-[90vh] tw:w-full tw:items-center tw:justify-center tw:overflow-scroll tw:bg-white tw:p-8 tw:shadow-xl tw:md:w-11/12 tw:md:rounded-2xl tw:lg:max-w-6xl"
                                initial={{
                                    opacity: 0,
                                    scale: 0.7,
                                    translateX: "-50%",
                            ...
                            >
                                <Dialog.Close className="tw:fixed tw:right-0 tw:top-0 tw:m-4 tw:cursor-pointer tw:rounded-full tw:bg-white tw:p-1 tw:text-secondary tw:shadow-inner tw:drop-shadow-md tw:hover:bg-secondary tw:hover:text-white tw:md:right-5 tw:md:top-5">
