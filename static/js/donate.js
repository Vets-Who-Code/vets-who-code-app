let kindfulWidget = kindfulWidget || {};
let childKindfulWindow;
...
window.onmessage = function(event) {
  if (event.data === 'kclosed') {
    if (
      ...
      let donateframe = document.getElementById('kindful-iframe');
    ...
    let redirectUrl = event.data.replace('kredirect|', '');
...
include('https://vetswhocode.kindful.com//javascripts/jquery.js', function() {
  ...
  $(document).ready(function() {
    let lookupType = $('script[data-embed-id="donate"]').data('lookup-type');
    let lookupValue = $('script[data-embed-id="donate"]').data('lookup-value');
    let noStyles = $('script[data-embed-id="donate"]').data('styles-off');

    if (lookupType == 'xpath') {
      let xpathValues = lookupValue.split('  ');

      $.each(xpathValues, function(index, value) {
        let $xpathElement = $(document).xpathEvaluate(value);
