...
function migrateBlogImages(applyChanges = false) {
  ...
  if (results.changed.length > 0) {
    ...
    if (applyChanges) {
      ...
      console.log('⚠️  IMPORTANT: Review the changes and test your blog posts before committing.\n');
    } else {
      console.log('ℹ️  This is a DRY RUN. No files were modified.');
      console.log('   To apply these changes, run: node scripts/migrate-blog-images.js --apply\n');
    }
  } else {
    console.log('✅ All blog posts are already using public IDs or don\'t have Cloudinary images.\n');
  }
