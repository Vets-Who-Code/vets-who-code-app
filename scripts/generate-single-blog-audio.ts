...
async function uploadToCloudinary(audioBuffer: Buffer, publicId: string): Promise<string> {
...
async function main() {
  ...
  const apiKey = process.env.GOOGLE_GENERATIVE_AI_API_KEY || process.env.GEMINI_API_KEY || process.env.GOOGLE_PRIVATE_KEY;
  ...
  console.log(`Generating audio overview for blog: ${blogSlug}`);
  ...
  const cloudinaryUrl = await uploadToCloudinary(audioBuffer, blogSlug);

  console.log('\n✅ Audio generated and uploaded successfully!');
  console.log(`\nCloudinary URL: ${cloudinaryUrl}`);
  console.log(`\nAdd this to your blog frontmatter:`);
  console.log(`audioUrl: "${cloudinaryUrl}"`);
}
